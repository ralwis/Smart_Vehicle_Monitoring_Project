...
function epochToJsDate(epochTime){
  ...
  const speeElement = document.getElementById("spee");
    ...
    if (user) {
      ...
      chartRef.on('value', snapshot =>{
        ...
        dbRef.orderByKey().limitToLast(chartRange).on('child_added', snapshot =>{
          var jsonData = snapshot.toJSON(); // example: {speed: 25.02, distance: 50.20, timestamp:1641317355}
          ...
          plotValues(chartD, timestamp, distance);
        });
      });
      ...
      dbRef.orderByKey().limitToLast(1).on('child_added', snapshot =>{
        ...
        var timestamp = jsonData.timestamp;
        // Update DOM elements
        speeElement.innerHTML = speed;
        distElement.innerHTML = distance;
      ...
      dbRef.orderByKey().limitToLast(1).on('child_added', snapshot =>{
        ...
        var gaugeD = createDistanceGauge();
        gaugeS.draw();
        gaugeS.value = speed;
        //gaugeD.value = distance;
        updateElement.innerHTML = epochToDateTime(timestamp);
      });
